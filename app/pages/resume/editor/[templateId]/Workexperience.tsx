...
const WorkExperience: React.FC<WorkExperienceProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<
    "work" | "internship" | "project" | "volunteer"
  >("work");
  ...
  const updateExperience = (
    ...
    setFormData(
      formData.map((exp) => (exp.id === id ? { ...exp, [field]: value } : exp))
    );
  };
  ...
  const removeTechnology = (expId: string, index: number) => {
    setFormData(
      formData.map((exp) => {
        if (exp.id === expId) {
          const newTechnologies = (exp.technologies || []).filter(
            (_, i) => i !== index
          );
          return { ...exp, technologies: newTechnologies };
        }
  ...
  useEffect(() => {
    const validateData = () => {
      const currentExperiences = formData.filter(
      ...
      return currentExperiences.every((exp) => {
        ...
        const hasValidDates =
          exp.current ||
          (exp.endDate && new Date(exp.startDate) <= new Date(exp.endDate));
        const hasDescription = exp.description.length >= 10;
        const hasAchievements =
          exp.achievements.length > 0 &&
          exp.achievements.every((a) => a.length > 0);
        ...
        return (
          hasBasicInfo && hasValidDates && hasDescription && hasAchievements
        );
      });
    };
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                  ...
                  <div className="col-span-2">
                    ...
                    <input
                      ...
                      className="w-full rounded-lg border border-gray-300 p-3 text-sm md:text-base text-gray-900 placeholder-gray-500 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-150 ease-in-out bg-white"
                      placeholder={
                        experience.type === "work"
                          ? "Software Engineer"
                          : experience.type === "internship"
                  ...
                  <div className="col-span-2">
                    ...
                    <textarea
                      ...
                      rows={4}
                      className="w-full rounded-lg border border-gray-300 p-3 text-sm md:text-base text-gray-900 placeholder-gray-500 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-150 ease-in-out bg-white resize-y min-h-[100px]"
                      placeholder={
                        experience.type === "work"
                      ...
                      <div key={idx} className="flex items-start space-x-2">
                        ...
                        {experience.achievements.length > 1 && (
                          <button
                            type="button"
                            onClick={() =>
                              removeAchievement(experience.id, idx)
                            }
                          ...
                          >
                            ...
                            {(experience.technologies?.length || 0) > 1 && (
                              <button
                                type="button"
                                onClick={() =>
                                  removeTechnology(experience.id, idx)
                                }
                                className="text-gray-400 hover:text-red-500 transition-colors"
                              >
                                <Trash2 size={16} className="md:w-4 md:h-4" />
